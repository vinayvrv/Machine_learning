...
def load_susy(trainsize=500, testsize=1000):
    ...
    #filename = 'C:\Third Sem\Machine Learning\Assignment_3\datasets\su.csv'
    dataset = loadcsv(filename)
    trainset, testset = splitdataset(dataset,trainsize, testsize)
    return trainset,testset


def load_card(trainsize=500, testsize=1000):
    """ A physics classification dataset """
    #filename = 'datasets/susysubset.csv'
    filename = 'datasets/card_data.csv'
    dataset = loadcsv(filename)
    trainset, testset = splitdataset(dataset,trainsize, testsize)
    return trainset,testset

def load_card1(trainsize=500, testsize=1000):
    """ A physics classification dataset """
    #filename = 'datasets/susysubset.csv'
    filename = 'datasets/card_data.csv'
    dataset = loadcsv(filename)
    trainset, testset = splitdataset1(dataset,trainsize, testsize)
    return trainset,testset
...
def splitdataset(dataset, trainsize, testsize, testdataset=None, featureoffset=None, outputfirst=None):
    ...
    #randindices = [i for i in range(dataset.shape[0])]
    #randindices = random.shuffle(randindices)
    ...
    if outputfirst is not None:
        ...
        outputlocation = 0
    ##########################
    a = np.arange(dataset.shape[0])
    np.random.shuffle(a)

    Xtrain = dataset[a[0:trainsize], featureoffset:featureend]
    ytrain=dataset[a[0:trainsize], outputlocation]
    Xtest = dataset[a[trainsize:trainsize+testsize],featureoffset:featureend]
    ytest = dataset[a[trainsize:trainsize+testsize],outputlocation]
...
def splitdataset1(dataset, trainsize, testsize, testdataset=None, featureoffset=None, outputfirst=None):
    """
    Splits the dataset into a train and test split
    If there is a separate testfile, it can be specified in testfile
    If a subset of features is desired, this can be specifed with featureinds; defaults to all
...
def splitdataset1(dataset, trainsize, testsize, testdataset=None, featureoffset=None, outputfirst=None):
    """
    this is used for selecting the tuning paramater, the data is already randomized using excel function manually and we
    need to pass in the entire data set for helecting the meta parameter in K fold cross validation
